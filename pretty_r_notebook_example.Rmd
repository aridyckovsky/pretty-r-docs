...
The chunk is labeled as `setup` and we've added the option `indclude=FALSE`, which will hide this code chunk from the prettified notebook output. The method inside this chunk, `knitr::opts_chunk$set()`, is the set of global options we want `knitr` to apply to every chunk in this notebook. For instance, we `warning = FALSE` makes sure that the end result does not include console warnings. While these warnings are great for development, we don't want to muddy our presentation with them!
